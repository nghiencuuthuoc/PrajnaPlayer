...
# >>> ADD: Natural sort helpers (put near other imports)
import re
import unicodedata

def _strip_accents(s: str) -> str:
    """Remove Vietnamese diacritics for stable A->Z sorting."""
    if s is None:
        return ""
...
def _natural_key(s):
    """Natural sort key: casefold + accent-stripped + split numbers."""
    s2 = _strip_accents(s).lower()
    ...
    for p in parts:
        if p.isdigit():
            ...
            if p:
                out.append(p)
    # ensure non-empty to avoid comparing [] vs []
    return out or [""]
# <<< END ADD
...
_TRACKNO_RE = re.compile(r"^\s*(\d{1,4})(?=\D|$)")

def _extract_track_no(s: str) -> Optional[int]:
    """Extract leading track number like '001' -> 1. Return None if not present."""
    try:
        m = _TRACKNO_RE.match(s or "")
        if not m:
            return None
...
def _extract_yt_id(s: str) -> Optional[str]:
    """Extract YouTube-like id inside brackets, e.g. '[7ZrhKFEJ6Fc]' -> '7ZrhKFEJ6Fc'."""
    try:
        m = _YT_ID_RE.search(s or "")
...
def _closest_base_to_audio(bases: List[str], audio_stem: str) -> Optional[str]:
    """Pick the best subtitle base for a given audio stem.

    Priority:
    1) Same YouTube id in [brackets] if present.
    2) Same leading track number (001, 090, ...) if present.
    3) Otherwise fall back to token / substring similarity.

    Safety: if audio has a strong identifier (yt id or track no) but we cannot match either,
    return None (avoid accidentally loading an unrelated subtitle).
    """
    if not bases:
        return None
    ...
    audio_yt = _extract_yt_id(audio_stem)
    audio_no = _extract_track_no(audio_stem)

    has_yt_match = bool(audio_yt) and any((audio_yt in b) for b in bases)
    has_no_match = (audio_no is not None) and any((_extract_track_no(b) == audio_no) for b in bases)

    # If we have identifiers but none can be matched at all, refuse to guess.
    if (audio_yt or audio_no is not None) and not (has_yt_match or has_no_match):
        return None

    candidates = list(bases)
    if has_yt_match:
        candidates = [b for b in candidates if audio_yt in b]
    if has_no_match:
        candidates = [b for b in candidates if _extract_track_no(b) == audio_no]
    ...
    for b in candidates:
        ...
        if bn == target:
            score = 100.0
        elif bn.startswith(target) or bn.endswith(target):
            score = 90.0
        elif target and target in bn:
            score = 80.0
        else:
        ...
        # Identifier boosts/penalties
        b_yt = _extract_yt_id(b)
        if audio_yt and b_yt:
            score += 25.0 if (b_yt == audio_yt) else -25.0
        elif audio_yt and b_yt and (b_yt != audio_yt):
            score -= 25.0

        b_no = _extract_track_no(b)
        if audio_no is not None and b_no is not None:
            score += 15.0 if (b_no == audio_no) else -10.0

        if score > best_score:
            best, best_score = b, score

    # Final safety threshold when we had identifiers: do not accept very weak matches.
    if (audio_yt or audio_no is not None) and best_score < 25.0:
        return None
...
class PrajnaPlayerApp:
    def __init__(self, root: tk.Tk):
        ...
        self.base_title = "PrajnaPlayer v18"
        ...
        # sort state for header-click
        self._last_sort_col = "title"
    ...
    def _set_now_playing_label(self, media_path: Optional[Path]):
        """Update the small 'Now:' label in the progress row."""
        if not hasattr(self, "now_playing_lbl"):
            return
        if not media_path:
            try:
                self.now_playing_lbl.config(text="")
            except Exception:
                pass
            return
        ...
        try:
            self.now_playing_lbl.config(text=f"Now: {name}")
        except Exception:
            pass
    ...
    def _build_progress_row(self):
        ...
        # Show currently playing media file (mp3/video)
        self.now_playing_lbl = ttk.Label(bar, text="", anchor="e")
        self.now_playing_lbl.pack(side=tk.RIGHT, padx=(8, 0))
    ...
    def _build_playlist_expander(self):
        ...
        self.tv.heading("title", text="Title", command=lambda: self._sort_by_column("title"))
        self.tv.heading("folder", text="Folder", command=lambda: self._sort_by_column("folder"))
        self.tv.heading("dur", text="Duration", command=lambda: self._sort_by_column("dur"))
        self.tv.heading("size", text="Size", command=lambda: self._sort_by_column("size"))
        self.tv.heading("mod", text="Modified", command=lambda: self._sort_by_column("mod"))
    ...
    def resort(self):
        ...
        if key == "Title (A→Z)":
            ...
            items.sort(key=lambda x: _natural_key(x["name"]), reverse=True)

        elif key == "Folder (A→Z)":
            items.sort(key=lambda x: _natural_key(x["folder"]))
        elif key == "Folder (Z→A)":
            items.sort(key=lambda x: _natural_key(x["folder"]), reverse=True)
            ...
            # None last
            items.sort(key=lambda it: (it["duration_ms"] is None, -(it["duration_ms"] or 0)))
        elif key == "Duration (Short→Long)":
            # None last
            items.sort(key=lambda it: (it["duration_ms"] is None, (it["duration_ms"] or 10**12)))
    ...
    def _sort_by_column(self, col: str):
        # toggle if same column; otherwise reset asc=True
        asc = True if self._last_sort_col != col else (not self._last_sort_asc)
        self._last_sort_col, self._last_sort_asc = col, asc

        # ensure items_all exists
        items = getattr(self, "items_all", [])
        if not isinstance(items, list):
            items = []

        if col == "title":
            items.sort(key=lambda x: _natural_key(x.get("name","")), reverse=not asc)
        elif col == "folder":
            items.sort(key=lambda x: _natural_key(x.get("folder","")), reverse=not asc)
        elif col == "dur":
            if asc:
                items.sort(key=lambda it: (it.get("duration_ms") is None, (it.get("duration_ms") or 10**12)))
            else:
                items.sort(key=lambda it: (it.get("duration_ms") is None, -(it.get("duration_ms") or 0)))
        elif col == "size":
            items.sort(key=lambda x: x.get("size", 0), reverse=not asc)
        elif col == "mod":
            items.sort(key=lambda x: x.get("mtime", 0), reverse=not asc)

        self.items_all = items

        # header arrows
        def _hdr(name, base):
            if name != col: return base
            return base + (" ▲" if asc else " ▼")

        self.tv.heading("title",  text=_hdr("title",  "Title"))
        self.tv.heading("folder", text=_hdr("folder", "Folder"))
        self.tv.heading("dur",    text=_hdr("dur",    "Duration"))
        self.tv.heading("size",   text=_hdr("size",   "Size"))
        self.tv.heading("mod",    text=_hdr("mod",    "Modified"))
    ...
    def _play_index(self, idx, resume_ms: int = 0):
        ...
        self._set_now_playing_label(path)
        if vlc:
    ...
    def _fuzzy_pick_for_audio(self, audio_path: Path) -> Tuple[Optional[Path], Optional[Path]]:
        """Last-resort subtitle picker (same folder).

        Unlike the old implementation (best EN + best VI independently), this keeps EN/VI
        coherent by first selecting the best *base* for the currently playing audio,
        then picking EN/VI files that belong to that same base.
        """
        folder = audio_path.parent
        audio_stem = audio_path.stem

        def _base_key(p: Path) -> str:
            # Prefer strict BASE.en.vtt pattern when available.
            b = _suffix2_base(p)
            if b:
                return b
            # Otherwise: use stem, but try to strip trailing language words.
            s = p.stem
            s = re.sub(r"(?i)[\s._-]*(english|eng|viet(?:namese)?|vietnamese|vietnam|vi|en)$", "", s).strip(" ._-")
            return s or p.stem
        ...
        for p in sorted(folder.glob("*")):
            ...
            if lang not in ("en", "vi"):
                continue
            base = _base_key(p)
            d = groups.setdefault(base, {})

            # Prefer .vtt over .srt when both exist.
            cur = d.get(lang)
            if cur is None:
                d[lang] = p
            else:
                if cur.suffix.lower() != ".vtt" and p.suffix.lower() == ".vtt":
                    d[lang] = p

        if not groups:
            return None, None

        best_base = _closest_base_to_audio(list(groups.keys()), audio_stem)
        if not best_base:
            return None, None
