...
def _prepare_vlc_runtime():
    """Khi chạy bản one-file: thêm đường dẫn DLL và trỏ VLC_PLUGIN_PATH tới ./plugins."""
    try:
        here = _bundle_dir()
        # Python 3.8+ trên Windows: thêm đường dẫn tra DLL
        if os.name == "nt" and hasattr(os, "add_dll_directory"):
            os.add_dll_directory(str(here))
        # Chỉ định thư mục plugin của VLC (được bundle vào ./plugins)
        plug = here / "plugins"
        if plug.exists():
...
def _bundle_dir() -> Path:
    # Thư mục chứa DATA đã bundle (đọc assets, icon…)
    if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
        return Path(sys._MEIPASS)
    return Path(__file__).parent

def _runtime_dir() -> Path:
    # Thư mục chạy thật (vị trí .exe); nơi ta tạo config_state/
...
class PrajnaPlayerApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        # Thay bằng:
        self.app_root = _bundle_dir()       # đọc assets từ bundle
        self.app_dir  = _runtime_dir() / "config_state"  # ghi JSON cạnh .exe
        self.app_dir.mkdir(exist_ok=True)
        self._migrate_old_jsons(_runtime_dir(), self.app_dir)  # migrate từ thư mục chạy
    ...
    def choose_center_image(self):
        initial = Path(self.current_folder or _bundle_dir())
...
def set_app_icon(root: tk.Tk):
    # Ưu tiên .ico cho Windows
    try:
        here = _bundle_dir()   # hoặc: Path(__file__).parent
        ico = here / "prajna.ico"
        if ico.exists():
    ...
    # Fallback PNG/JPG (giữ nguyên như cũ)
    try:
        if Image is not None and ImageTk is not None:
            for name in ("prajna.png", "buddha.png", "logo.png"):
                p = here / name
...
def main():
    _prepare_vlc_runtime()      # <-- thêm dòng này
    root = tk.Tk()
    set_app_icon(root)
    app = PrajnaPlayerApp(root)
    root.mainloop()
