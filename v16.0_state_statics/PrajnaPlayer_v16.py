...
# -*- coding: utf-8 -*-
"""
PrajnaPlayer v16 – State & Static cache + Dual Subtitles + Smart-Hold

Cập nhật mới:
STATE
- state_<hash>.json: (đặt cạnh app) 1 file cho mỗi thư mục nhạc (hash từ đường dẫn tuyệt đối).
  Lưu: base_folder, current_index, volume, position_sec.
  Tự lưu khi Play và định kỳ (mặc định mỗi 5s).
- state_recent.json: con trỏ đến thư mục dùng gần nhất để khôi phục khi mở app.

STATIC
- static.json: nằm trong chính thư mục nhạc; cache danh sách bài + metadata:
  {
    "title": <tên thư mục>,
    "base_folder": <đường dẫn tuyệt đối>,
    "created_at": <ISO8601>,
    "tracks": [{"path":..., "title":..., "size":..., "mtime":...}, ...]
  }
- Ghi an toàn/atomic: dùng static.lock + file tạm rồi replace.
- Có chế độ read-only để không ghi (toggle trong UI hoặc config).

Giữ đầy đủ tính năng trước đó:
- Auto-load phụ đề EN/VI theo nhiều chiến lược (exact/strict/index/closest/fuzzy)
- Hiển thị 2 dòng phụ đề (EN trên, VI dưới)
- Smart-Hold: MIN_HOLD + PER_CHAR + LINGER (không tràn cue sau)
"""
...
def _alignment_diagnostics(en_cues, vi_cues) -> Tuple[str, float, Optional[float]]:
    """Trả (status_text, overlap_ratio, median_offset_s)."""
...
class PrajnaPlayerApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("PrajnaPlayer v16 · PharmApp")
        ...
        # Sub-index (base -> {'en':Path,'vi':Path}), làm mới khi scan folder
        ...
        # STATIC cache mode (read-only toggle)
        self.static_read_only = tk.BooleanVar(value=bool(self._cfg.get("static_read_only", False)))

        # State autosave interval (ms)
        self.state_autosave_ms = int(self._cfg.get("state_autosave_ms", 5000))
        self._last_state_dump_ts = 0.0
        self._pending_seek: Optional[Tuple[int, int]] = None  # (index, ms) khi restore
        ...
        # Try restore recent folder/state (không auto-play; chọn sẵn bài & vị trí)
        self._maybe_restore_recent()

        # periodic
        self.root.after(300, self._tick)
        self.root.after(self.state_autosave_ms, self._autosave_tick)
    ...
    def _save_config(self):
        try:
            ...
            self._cfg["static_read_only"]= bool(self.static_read_only.get())
            self._cfg["state_autosave_ms"]= int(self.state_autosave_ms)
    ...
    # ----- Atomic IO helpers -----
    def _atomic_write_json(self, path: Path, data: dict, use_lock: bool=False, lock_path: Optional[Path]=None):
        """Ghi JSON an toàn: file tạm + replace. Có thể dùng lock file (exclusive)."""
        try:
            if use_lock and lock_path:
                # tạo lock (exclusive). nếu đã có thì bỏ qua
                try:
                    with open(lock_path, "x") as _:
                        pass
                except FileExistsError:
                    return  # có tiến trình khác đang ghi -> bỏ qua
            ...
            if use_lock and lock_path and lock_path.exists():
                try: lock_path.unlink()
                except Exception: pass

    def _folder_hash(self, folder: str) -> str:
        h = hashlib.sha1(folder.encode("utf-8")).hexdigest()
        return h[:16]

    def _state_path_for(self, folder: str) -> Path:
        return self.app_dir / f"state_{self._folder_hash(folder)}.json"

    def _recent_path(self) -> Path:
        return self.app_dir / "state_recent.json"

    # ----- STATIC cache -----
    def _write_static_cache(self, folder: Path):
        if self.static_read_only.get():
            return
        try:
            tracks = []
            ...
            payload = {
                "title": folder.name,
                "base_folder": str(folder.resolve()),
                "created_at": datetime.now().isoformat(timespec="seconds"),
                "tracks": tracks,
            }
            target = folder / "static.json"
            lock   = folder / "static.lock"
            self._atomic_write_json(target, payload, use_lock=True, lock_path=lock)
        except Exception:
            # Im lặng để không làm phiền khi chỉ là cache
            pass

    # ----- STATE save/restore -----
    def _collect_state(self) -> Optional[dict]:
        if not self.current_folder:
            return None
        pos_ms = self._current_play_ms()
        position_sec = max(0, int(pos_ms/1000)) if pos_ms >= 0 else 0
        st = {
    ...
    def _save_state(self, force: bool=False):
        if not self.current_folder:
            return
        now = time.time()
        if not force and (now - self._last_state_dump_ts) < (self.state_autosave_ms/1000.0 - 0.2):
            return
        st = self._collect_state()
        if not st: return
        try:
            path = self._state_path_for(self.current_folder)
            self._atomic_write_json(path, st, use_lock=False)
    ...
    def _save_recent(self, folder: str):
        try:
            rec = {"base_folder": folder, "saved_at": datetime.now().isoformat(timespec="seconds")}
            self._atomic_write_json(self._recent_path(), rec, use_lock=False)
        except Exception:
            pass

    def _load_state_for(self, folder: str) -> Optional[dict]:
        try:
            path = self._state_path_for(folder)
            if path.exists():
    ...
    def _maybe_restore_recent(self):
        try:
            rp = self._recent_path()
            if not rp.exists(): return
            rec = json.loads(rp.read_text(encoding="utf-8"))
            folder = rec.get("base_folder")
            if not folder or not Path(folder).exists(): return
            # scan + try load per-folder state
            self.current_folder = folder
            self.scan_folder(folder)
            self.resort(); self.apply_filter()
            st = self._load_state_for(folder)
            if st:
                vol = int(st.get("volume", self.volume.get()))
                self.volume.set(vol); self.set_volume()
                idx = int(st.get("current_index", -1))
                pos = int(st.get("position_sec", 0)) * 1000
                # Nếu index hợp lệ: chọn sẵn trên Treeview; khi user Play sẽ seek
                if 0 <= idx < len(self.items_all):
                    ...
                    try:
                        ...
                        pass
                    # lưu pending seek cho lần play kế
                    self._pending_seek = (idx, pos)
        except Exception:
            pass
    ...
    def _build_subtitle_row_dual(self):
        ...
        # Toggle STATIC read-only
        self.btn_static_ro = tk.Button(top, text=f"Static: {'RO' if self.static_read_only.get() else 'RW'}",
                                       command=self._toggle_static_ro)
        style_btn(self.btn_static_ro)
        self.btn_static_ro.pack(side=tk.RIGHT, padx=(6, 0))
    ...
    def _toggle_static_ro(self):
        self.static_read_only.set(not self.static_read_only.get())
        self.btn_static_ro.config(text=f"Static: {'RO' if self.static_read_only.get() else 'RW'}")
        self._save_config()
    ...
    def _build_sub_index(self, root: Path):
        """Quét toàn bộ thư mục đã mở để lập chỉ mục phụ đề strict pair."""
    ...
    # ----- Subtitle handling -----
    @staticmethod
    def _guess_lang_from_name(name: str) -> str:
        n = name.lower()
    ...
    def _auto_pair_for(self, picked: Path, expect: str) -> Tuple[Optional[Path], Optional[Path]]:
        """Fallback pairing khi không có strict base."""
        ...
        for p in folder.glob("*"):
            ...
            inter = len(_token_set(nm) & _token_set(base_norm))
    ...
    def _exact_pair_for_audio(self, audio_path: Path) -> Tuple[Optional[Path], Optional[Path]]:
        """Ưu tiên cặp <stem>.en.(vtt|srt) và <stem>.vi.(vtt|srt) cạnh file audio/video."""
    ...
    def _auto_load_subtitle_for(self, audio_path: Path):
        """Load phụ đề theo thứ tự: exact same folder → strict same folder → sub-index exact → sub-index closest → fuzzy."""
        folder = audio_path.parent
    ...
    def _lookup_cue_text(self, cues, t_ms_adj: int):
        """
        Giữ phụ đề lâu hơn theo 3 tiêu chí:
        - LINGER ms sau khi hết cue
        - MIN_HOLD tối thiểu (ms)
        - PER_CHAR (ms/ký tự, bỏ khoảng trắng)
        Tất cả đều bị chặn để không đè lên cue kế tiếp.
        """
        if not cues:
            return None
        ...
        def eff_end(i, a, b, s, next_start):
            ...
            # Không vượt cue kế tiếp (chừa biên 40ms)
        ...
        # Cho linger/min-hold ở cue cuối
        la, lb, ls = cues[-1]
    ...
    def open_folder(self):
        ...
        # sau khi scan, ghi/static cache (nếu RW)
        self._write_static_cache(Path(folder))
    ...
    def open_static_file(self):
        ...
        if self.current_folder:
            ...
            self._write_static_cache(Path(self.current_folder))
        self.resort(); self.apply_filter()
    ...
    def _play_index(self, idx):
        ...
        # apply pending seek nếu đang khôi phục
        if self._pending_seek and self._pending_seek[0] == idx:
            ms = int(self._pending_seek[1])
            self.root.after(700, lambda: self._try_seek_ms(ms))
            self._pending_seek = None

        # auto load dual subtitles
        self._auto_load_subtitle_for(path)

        # lưu STATE ngay khi Play
        self._save_recent(str(path.parent))
        self._save_state(force=True)

        # Cập nhật duration vào bảng nếu VLC đã tính xong
        self.root.after(700, lambda: self._maybe_fill_duration(idx))

    def _try_seek_ms(self, ms: int):
