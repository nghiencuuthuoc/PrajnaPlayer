...
BTN_PRIMARY = "#C9792B"
BTN_PRIMARY_ACTIVE = "#D88A40"
BTN_PRIMARY_PRESSED = "#A76022"
BTN_SECONDARY = "#D9CDBF"
BTN_SECONDARY_ACTIVE = "#E8DED3"
BTN_SECONDARY_PRESSED = "#C7B8A7"
BTN_ACCENT = "#B88A3B"
BTN_ACCENT_ACTIVE = "#C89B4B"
BTN_ACCENT_PRESSED = "#9A712E"
BTN_ACTIVE = "#3F6F63"
BTN_ACTIVE_HOVER = "#4E8174"
BTN_ACTIVE_PRESSED = "#31594F"
BTN_SYSTEM = "#8A5A44"
BTN_SYSTEM_ACTIVE = "#9D6A53"
BTN_SYSTEM_PRESSED = "#714634"
...
BUTTON_ROLE_STYLES = {
    "primary": {
        ...
        "activeforeground": BTN_TEXT_LIGHT,
        "border": BTN_PRIMARY_PRESSED,
    },
    "secondary": {
        "bg": BTN_SECONDARY,
        "fg": BTN_TEXT_DARK,
        "activebackground": BTN_SECONDARY_ACTIVE,
        "activeforeground": BTN_TEXT_DARK,
        "border": BTN_SECONDARY_PRESSED,
    },
    "accent": {
        "bg": BTN_ACCENT,
        "fg": BTN_TEXT_LIGHT,
        "activebackground": BTN_ACCENT_ACTIVE,
        "activeforeground": BTN_TEXT_LIGHT,
        "border": BTN_ACCENT_PRESSED,
    },
    "active": {
        "bg": BTN_ACTIVE,
        "fg": BTN_TEXT_LIGHT,
        "activebackground": BTN_ACTIVE_HOVER,
        "activeforeground": BTN_TEXT_LIGHT,
        "border": BTN_ACTIVE_PRESSED,
    },
    "system": {
        "bg": BTN_SYSTEM,
        "fg": BTN_TEXT_LIGHT,
        "activebackground": BTN_SYSTEM_ACTIVE,
        "activeforeground": BTN_TEXT_LIGHT,
        "border": BTN_SYSTEM_PRESSED,
    },
}
...
# ----- UI helpers -----
def _button_role_palette(role: str = "secondary", active: bool = False) -> dict:
    if active:
        return BUTTON_ROLE_STYLES["active"]
    return BUTTON_ROLE_STYLES.get(role, BUTTON_ROLE_STYLES["secondary"])
...
def apply_pharmapp_theme(root: tk.Tk):
    ...
    s.configure("TLabelframe.Label", background=BG, foreground=FG, font=("Arial", 10, "bold"))
    s.configure("TLabel", background=BG, foreground=FG, font=("Arial", 11))
    s.configure("TCheckbutton", background=BG, foreground=FG, font=("Arial", 10))
    ...
    s.configure(
        ...
        foreground=FG,
        bordercolor=BORDER,
        lightcolor=BTN_SECONDARY_ACTIVE,
        darkcolor=BTN_SECONDARY_PRESSED,
        focusthickness=1,
        focuscolor=FOCUS,
    )
    s.map(
        "TButton",
        background=[("pressed", BTN_SECONDARY_PRESSED), ("active", BTN_SECONDARY_ACTIVE)],
        foreground=[("disabled", "#7A6C5E")],
    )
    s.configure(
        "Expander.TButton",
        background=BG,
    ...
    s.configure(
        ...
        padding=(10, 5),
        font=("Arial", 9, "bold"),
        background=BTN_SYSTEM,
        foreground=BTN_TEXT_LIGHT,
        bordercolor=BTN_SYSTEM_PRESSED,
        lightcolor=BTN_SYSTEM_ACTIVE,
        darkcolor=BTN_SYSTEM_PRESSED,
    )
    s.map(
        "PanelSystem.TButton",
        background=[("pressed", BTN_SYSTEM_PRESSED), ("active", BTN_SYSTEM_ACTIVE)],
        foreground=[("pressed", BTN_TEXT_LIGHT), ("active", BTN_TEXT_LIGHT)],
    )
    s.configure(
        "Treeview",
        background="white",
    ...
    s.map("Treeview", background=[("selected", "#E2C686")], foreground=[("selected", FG)])
    s.configure(
        "Horizontal.TScale",
        background=BG,
        troughcolor="#D8CCBC",
        bordercolor=BORDER,
        lightcolor=BTN_PRIMARY_ACTIVE,
        darkcolor=BTN_PRIMARY_PRESSED,
    )


def apply_button_role(b: tk.Button, role: Optional[str] = None, active: Optional[bool] = None) -> None:
    role = role or getattr(b, "_prajna_role", "secondary")
    active = getattr(b, "_prajna_active", False) if active is None else bool(active)
    palette = _button_role_palette(role, active=active)
    b._prajna_role = role
    b._prajna_active = active
    b.configure(
        bg=palette["bg"],
        fg=palette["fg"],
        activebackground=palette["activebackground"],
        activeforeground=palette["activeforeground"],
        disabledforeground="#7A6C5E",
...
def style_btn(b: tk.Button, role: str = "secondary") -> None:
    b._prajna_role = role
    b._prajna_active = False
    apply_button_role(b, role=role, active=False)


def set_button_active(b: tk.Button, is_active: bool, role_when_off: Optional[str] = None) -> None:
    if role_when_off is not None:
        b._prajna_role = role_when_off
    apply_button_role(b, role=getattr(b, "_prajna_role", "secondary"), active=is_active)
...
def _suffix2_base(p: Path) -> Optional[str]:
    """
    Hỗ trợ cả:
      - BASE.en.srt / BASE.vi.vtt
      - BASE_en.srt / BASE_vi.vtt
      - BASE-en.srt / BASE-vi.vtt
    Trả về 'BASE' nếu nhận diện được, ngược lại None.
    """
    name = p.name
    suf = Path(name).suffixes  # ví dụ ['.en', '.vtt'] hoặc ['.srt']
    ...
    # dạng BASE_<lang>.<ext> / BASE-<lang>.<ext>
    if p.suffix.lower() in ('.vtt', '.srt'):
        stem = p.stem
        m = re.match(r'^(.*?)[_\-](en|vi)$', stem, flags=re.IGNORECASE)
        if m:
            base = (m.group(1) or '').rstrip(' ._-')
            return base or None

    return None


def _suffix2_lang(p: Path) -> Optional[str]:
    """Return 'en'/'vi' for supported dual-sub naming styles, else None."""
    name = p.name.lower()
    suf = Path(name).suffixes
    if len(suf) >= 2 and suf[-1] in ('.vtt', '.srt') and suf[-2].lstrip('.') in ('en', 'vi'):
        return suf[-2].lstrip('.')
    if p.suffix.lower() in ('.vtt', '.srt'):
        m = re.match(r'^(.*?)[_\-](en|vi)$', p.stem, flags=re.IGNORECASE)
        if m:
...
class CollapsibleSection(ttk.Frame):
    def __init__(self, master, title: str, open_=True, style: str = "CollapsibleHeader.TButton"):
        super().__init__(master)
        self.title = title
        self.style = style
        self.var_open = tk.BooleanVar(value=bool(open_))
        self.header_btn = ttk.Button(self, style=self.style, command=self.toggle)
        self.header_btn.pack(fill=tk.X)
        self.body = ttk.Frame(self)
        if self.var_open.get():
            self.body.pack(fill=tk.BOTH, expand=True, pady=(2, 0))
        self._refresh_header()

    def _refresh_header(self):
        arrow = "▾" if self.var_open.get() else "▸"
        self.header_btn.configure(text=f"{arrow} {self.title}")

    def set_open(self, open_: bool):
        open_ = bool(open_)
        if self.var_open.get() == open_:
            self._refresh_header()
            return
        self.var_open.set(open_)
        if open_:
            self.body.pack(fill=tk.BOTH, expand=True, pady=(2, 0))
        else:
    ...
    def toggle(self):
        self.set_open(not self.var_open.get())
...
class PrajnaPlayerApp:
    def __init__(self, root: tk.Tk):
        ...
        self.playback_rate = tk.DoubleVar(value=float(self._cfg.get("playback_rate", 1.0)))
        self.now_playing_var = tk.StringVar(value="")  # UI: show current track

        saved_panels = self._cfg.get("panel_visibility", {})
        if not isinstance(saved_panels, dict):
            saved_panels = {}
        self._panel_visible = {
            "left": bool(saved_panels.get("left", True)),
            "right": bool(saved_panels.get("right", True)),
            "top": bool(saved_panels.get("top", True)),
            "bottom": bool(saved_panels.get("bottom", True)),
        }
        self._panel_toggle_buttons = {}
        ...
        self._build_dock_shell()
        self._build_toggle_toolbar()
        self._build_hero_paned(self.top_panel_host)
        self._build_bottom_filters_bar(self.center_panel_host)
        self._build_progress_row(self.center_panel_host)
        self._build_subtitle_row_dual(self.center_panel_host)
        self._build_playlist_expander(self.bottom_panel_host)
        self._apply_panel_visibility(initial=True)
    ...
    def _save_config(self):
        try:
            ...
            self._cfg["playback_rate"]   = float(self.playback_rate.get())
            ...
            self._cfg["panel_visibility"]   = dict(self._panel_visible)
    ...
    # --- Dock shell / panel visibility ---
    def _build_dock_shell(self):
        ...
        self.app_shell.grid_rowconfigure(3, weight=1)

        self.toolbar_host = tk.Frame(self.app_shell, bg=TOOLBAR_BG, highlightthickness=1, highlightbackground=BORDER)
        self.toolbar_host.grid(row=0, column=0, sticky="ew")

        self.top_panel_host = ttk.Frame(self.app_shell)
        self.top_panel_host.grid(row=1, column=0, sticky="nsew")

        self.center_panel_host = ttk.Frame(self.app_shell)
        self.center_panel_host.grid(row=2, column=0, sticky="ew")

        self.bottom_panel_host = ttk.Frame(self.app_shell)
        self.bottom_panel_host.grid(row=3, column=0, sticky="nsew")

    def _build_toggle_toolbar(self):
        bar = tk.Frame(self.toolbar_host, bg=TOOLBAR_BG)
        bar.pack(fill=tk.X, padx=6, pady=6)

        self.btn_toggle_left = ttk.Button(bar, style="PanelToggle.TButton", command=self.toggle_left_panel, width=10)
        self.btn_toggle_right = ttk.Button(bar, style="PanelToggle.TButton", command=self.toggle_right_panel, width=10)
        self.btn_toggle_top = ttk.Button(bar, style="PanelToggle.TButton", command=self.toggle_top_panel, width=10)
        self.btn_toggle_bottom = ttk.Button(bar, style="PanelToggle.TButton", command=self.toggle_bottom_panel, width=12)
        self.btn_reset_panels = ttk.Button(bar, text="Reset Layout", style="PanelSystem.TButton", command=self.reset_panel_layout)

        self.btn_toggle_left.pack(side=tk.LEFT, padx=(0, 4))
        self.btn_toggle_right.pack(side=tk.LEFT, padx=(0, 4))
        self.btn_toggle_top.pack(side=tk.LEFT, padx=(0, 4))
        self.btn_toggle_bottom.pack(side=tk.LEFT, padx=(0, 8))
        self.btn_reset_panels.pack(side=tk.LEFT)

        self._panel_toggle_buttons = {
            "left": self.btn_toggle_left,
            "right": self.btn_toggle_right,
            "top": self.btn_toggle_top,
            "bottom": self.btn_toggle_bottom,
        }
        self._refresh_panel_toggle_buttons()

    def _panel_button_text(self, region: str) -> str:
        visible = bool(self._panel_visible.get(region, True))
        if region == "left":
            return "◀ Left" if visible else "▶ Left"
        if region == "right":
            return "Right ▶" if visible else "Right ◀"
        if region == "top":
            return "▲ Top" if visible else "▼ Top"
        if region == "bottom":
            return "Bottom ▼" if visible else "Bottom ▲"
        return region.title()

    def _refresh_panel_toggle_buttons(self):
        for region, btn in self._panel_toggle_buttons.items():
            try:
                btn.configure(text=self._panel_button_text(region))
            except Exception:
                pass

    def _apply_panel_visibility(self, initial: bool = False):
        self._set_top_panel_visible(self._panel_visible.get("top", True), refresh=False)
        self._set_bottom_panel_visible(self._panel_visible.get("bottom", True), refresh=False)
        self._set_left_panel_visible(self._panel_visible.get("left", True), refresh=False)
        self._set_right_panel_visible(self._panel_visible.get("right", True), refresh=False)
        if initial:
            self.root.after(100, self._refresh_panel_toggle_buttons)
        else:
            self._refresh_panel_toggle_buttons()

    def _set_top_panel_visible(self, visible: bool, refresh: bool = True):
        self._panel_visible["top"] = bool(visible)
        if self._panel_visible["top"]:
            self.top_panel_host.grid()
        else:
            self.top_panel_host.grid_remove()
        if refresh:
            self._refresh_panel_toggle_buttons()

    def _set_bottom_panel_visible(self, visible: bool, refresh: bool = True):
        self._panel_visible["bottom"] = bool(visible)
        if self._panel_visible["bottom"]:
            self.bottom_panel_host.grid()
        else:
            self.bottom_panel_host.grid_remove()
        if refresh:
            self._refresh_panel_toggle_buttons()

    def _set_left_panel_visible(self, visible: bool, refresh: bool = True):
        self._panel_visible["left"] = bool(visible)
        if hasattr(self, "hero") and hasattr(self, "p_left"):
            panes = set(self.hero.panes())
            pane_id = str(self.p_left)
            if self._panel_visible["left"] and pane_id not in panes:
                try:
                    self.hero.insert(0, self.p_left, weight=1)
                except Exception:
                    self.hero.add(self.p_left, weight=1)
            elif (not self._panel_visible["left"]) and pane_id in panes:
                try:
                    self.hero.forget(self.p_left)
                except Exception:
                    pass
        if refresh:
            self._refresh_panel_toggle_buttons()

    def _set_right_panel_visible(self, visible: bool, refresh: bool = True):
        self._panel_visible["right"] = bool(visible)
        if hasattr(self, "hero") and hasattr(self, "p_right"):
            panes = set(self.hero.panes())
            pane_id = str(self.p_right)
            if self._panel_visible["right"] and pane_id not in panes:
                try:
                    self.hero.add(self.p_right, weight=1)
                except Exception:
                    pass
            elif (not self._panel_visible["right"]) and pane_id in panes:
                try:
                    self.hero.forget(self.p_right)
                except Exception:
                    pass
        if refresh:
            self._refresh_panel_toggle_buttons()
    ...
    def toggle_left_panel(self):
        self._set_left_panel_visible(not self._panel_visible.get("left", True))

    def toggle_right_panel(self):
        self._set_right_panel_visible(not self._panel_visible.get("right", True))

    def toggle_top_panel(self):
        self._set_top_panel_visible(not self._panel_visible.get("top", True))

    def toggle_bottom_panel(self):
        self._set_bottom_panel_visible(not self._panel_visible.get("bottom", True))
    ...
    def reset_panel_layout(self):
        ...
        for attr in (
            "left_section_library", "left_section_view",
            "center_art_section",
            "right_section_transport", "right_section_speed",
            "bottom_playlist_section",
        ):
    ...
    def _bind_shortcuts(self):
        ...
        self.root.bind("<Alt-bracketleft>", lambda e: self._bump_playback_rate(-0.1))
        self.root.bind("<Alt-bracketright>", lambda e: self._bump_playback_rate(+0.1))
        self.root.bind("<Alt-backslash>", lambda e: self.set_playback_rate(1.0))

        self.root.bind("<Control-Alt-l>", lambda e: self.toggle_left_panel())
        self.root.bind("<Control-Alt-r>", lambda e: self.toggle_right_panel())
        self.root.bind("<Control-Alt-t>", lambda e: self.toggle_top_panel())
        self.root.bind("<Control-Alt-b>", lambda e: self.toggle_bottom_panel())
        self.root.bind("<Control-Alt-s>", lambda e: self.reset_panel_layout())
    ...
    def _bump_sub_font(self, delta):
        ...
        self._relayout_sub_labels()

    def _refresh_speed_label(self):
        try:
            r = float(self.playback_rate.get())
        except Exception:
            r = 1.0
        r = max(0.1, min(4.0, r))
        txt = f"{r:.2f}x".rstrip("0").rstrip(".")
        if hasattr(self, "speed_status_lbl") and self.speed_status_lbl:
            self.speed_status_lbl.config(text=f"Speed: {txt}")
        self._refresh_speed_button_visuals()

    def _refresh_speed_button_visuals(self):
        try:
            r = max(0.1, min(4.0, float(self.playback_rate.get())))
        except Exception:
            r = 1.0
        mapping = (
            (getattr(self, "_btn_speed_dn", None), False, "accent"),
            (getattr(self, "_btn_speed_up", None), False, "accent"),
            (getattr(self, "_btn_speed_1x", None), abs(r - 1.0) < 0.001, "secondary"),
            (getattr(self, "_btn_speed_fast", None), abs(r - 2.0) < 0.001, "secondary"),
        )
        for btn, is_active, off_role in mapping:
            if btn:
                set_button_active(btn, is_active, role_when_off=off_role)

    def _refresh_transport_toggle_visuals(self):
        if getattr(self, "_btn_repeat", None):
            set_button_active(self._btn_repeat, bool(getattr(self, "is_repeat", False)), role_when_off="secondary")
        if getattr(self, "_btn_shuffle", None):
            set_button_active(self._btn_shuffle, bool(getattr(self, "is_shuffle", False)), role_when_off="secondary")

    def _refresh_sub_toggle_visual(self):
        if getattr(self, "btn_sub_toggle", None):
            try:
                enabled = bool(self.sub_enabled.get())
            except Exception:
                enabled = True
            set_button_active(self.btn_sub_toggle, enabled, role_when_off="secondary")
    ...
    def set_playback_rate(self, rate: float):
        try:
            ...
            rate = 1.0
        rate = max(0.1, min(4.0, round(rate, 2)))
        self.playback_rate.set(rate)
        self._refresh_speed_label()
        self._apply_playback_rate()

    def _bump_playback_rate(self, delta: float):
        try:
            cur = float(self.playback_rate.get())
        except Exception:
            cur = 1.0
        self.set_playback_rate(cur + float(delta))

    def _apply_playback_rate(self):
        if not (self.player and vlc):
            return
        try:
            r = max(0.1, min(4.0, float(self.playback_rate.get())))
    ...
    def _relayout_sub_one(self, box, main_lbl, shadow_lbl):
        try:
            box.update_idletasks()
            h = max(main_lbl.winfo_reqheight(), shadow_lbl.winfo_reqheight()) + 3
            box.configure(height=max(8, h))
            shadow_lbl.place(relx=0.5, x=1, y=1, anchor="n")
            main_lbl.place(relx=0.5, x=0, y=0, anchor="n")
        except Exception:
            pass

    def _relayout_sub_labels(self):
        if not hasattr(self, "subtitle_en_lbl"):
            return
        self._relayout_sub_one(self.subtitle_en_box, self.subtitle_en_lbl, self.subtitle_en_shadow_lbl)
        self._relayout_sub_one(self.subtitle_vi_box, self.subtitle_vi_lbl, self.subtitle_vi_shadow_lbl)
    ...
    # --- Build UI ---
    def _build_hero_paned(self, parent=None):
        parent = parent or self.top_panel_host
        self.hero = ttk.Panedwindow(parent, orient=tk.HORIZONTAL)
        self.hero.pack(fill=tk.BOTH, expand=True, padx=6, pady=(8, 4))

        self.p_left = ttk.Frame(self.hero)
        self._populate_left_controls(self.p_left)
        self.hero.add(self.p_left, weight=1)

        self.p_center = ttk.Frame(self.hero)
        self.center_art_section = CollapsibleSection(self.p_center, "Artwork", open_=True)
        self.center_art_section.pack(fill=tk.BOTH, expand=True, padx=4, pady=2)
        center_body = ttk.Frame(self.center_art_section.body)
        center_body.pack(fill=tk.BOTH, expand=True)
        self._load_center_image(center_body)
        self.hero.add(self.p_center, weight=2)

        self.p_right = ttk.Frame(self.hero)
        self._populate_right_controls(self.p_right)
        self.hero.add(self.p_right, weight=1)

    def _build_bottom_filters_bar(self, parent=None):
        parent = parent or self.center_panel_host
        ...
        btn_clear = tk.Button(left, text="Clear [Esc]", command=self.clear_filter); style_btn(btn_clear, role="secondary")
        btn_clear.pack(side=tk.LEFT, padx=(6, 12))

        btn_img = tk.Button(left, text="Set Image [Ctrl+I]", command=self.choose_center_image); style_btn(btn_img, role="primary")
        btn_img.pack(side=tk.LEFT, padx=(0, 12))

        btn_img_rand = tk.Button(left, text="Random Img", command=self.shuffle_center_image); style_btn(btn_img_rand, role="accent")
        ...
        chk = tk.Checkbutton(right, text="Static write", bg=BG, activebackground=BG, variable=self.allow_write_static, onvalue=True, offvalue=False)
        chk.pack(side=tk.LEFT, padx=(12,0))

    def _build_progress_row(self, parent=None):
        parent = parent or self.center_panel_host
        bar = tk.Frame(parent, bg=BG)
        bar.pack(fill=tk.X, padx=8, pady=(0, 4))
    ...
    def _build_subtitle_row_dual(self, parent=None):
        parent = parent or self.center_panel_host
        outer = tk.Frame(parent, bg=BG); outer.pack(fill=tk.X, padx=8, pady=(0, 6))
        ...
        btn_load = tk.Button(top, text="Load Sub [Ctrl+L]", command=self.load_subtitle_manual); style_btn(btn_load, role="primary")
        btn_load.pack(side=tk.LEFT, padx=(12, 6))

        self.btn_sub_toggle = tk.Button(top, text="Sub: On [Ctrl+K]", command=self.toggle_sub_enabled); style_btn(self.btn_sub_toggle, role="secondary")
        self.btn_sub_toggle.pack(side=tk.LEFT, padx=(0, 12))

        btn_dec = tk.Button(top, text="A− [Ctrl+-]", command=lambda: self._bump_sub_font(-2)); style_btn(btn_dec, role="accent")
        btn_inc = tk.Button(top, text="A+ [Ctrl+=]", command=lambda: self._bump_sub_font(+2)); style_btn(btn_inc, role="accent")
        btn_dec.pack(side=tk.LEFT, padx=(0, 4)); btn_inc.pack(side=tk.LEFT, padx=(0, 12))

        btn_delay_m = tk.Button(top, text="Delay −5s", command=lambda: self.sub_delay_ms.set(self.sub_delay_ms.get()-5000)); style_btn(btn_delay_m, role="secondary")
        btn_delay_p = tk.Button(top, text="Delay +5s", command=lambda: self.sub_delay_ms.set(self.sub_delay_ms.get()+5000)); style_btn(btn_delay_p, role="secondary")
        ...
        self._refresh_sub_toggle_visual()

        self.subtitle_en_box = tk.Frame(outer, bg=BG, height=30)
        self.subtitle_vi_box = tk.Frame(outer, bg=BG, height=30)
        self.subtitle_en_box.pack(fill=tk.X, pady=(6, 0))
        self.subtitle_vi_box.pack(fill=tk.X, pady=(2, 0))
        self.subtitle_en_box.pack_propagate(False)
        self.subtitle_vi_box.pack_propagate(False)

        self.subtitle_en_shadow_lbl = tk.Label(self.subtitle_en_box, text="", bg=BG, fg="#111111",
                                               font=self.sub_font, justify="center", wraplength=1200)
        self.subtitle_en_lbl = tk.Label(self.subtitle_en_box, text="", bg=BG, fg="#FFD700",
                                        font=self.sub_font, justify="center", wraplength=1200)
        self.subtitle_vi_shadow_lbl = tk.Label(self.subtitle_vi_box, text="", bg=BG, fg="#111111",
                                               font=self.sub_font, justify="center", wraplength=1200)
        self.subtitle_vi_lbl = tk.Label(self.subtitle_vi_box, text="", bg=BG, fg="#90EE90",
                                        font=self.sub_font, justify="center", wraplength=1200)

        self.subtitle_en_shadow_lbl.place(relx=0.5, x=1, y=1, anchor="n")
        self.subtitle_en_lbl.place(relx=0.5, x=0, y=0, anchor="n")
        self.subtitle_vi_shadow_lbl.place(relx=0.5, x=1, y=1, anchor="n")
        self.subtitle_vi_lbl.place(relx=0.5, x=0, y=0, anchor="n")
        self._relayout_sub_labels()

    def _build_playlist_expander(self, parent=None):
        parent = parent or self.bottom_panel_host
        holder = ttk.Frame(parent)
        holder.pack(fill=tk.BOTH, expand=True, padx=8, pady=(2, 8))

        self.bottom_playlist_section = CollapsibleSection(holder, "Playlist", open_=True)
        self.bottom_playlist_section.pack(fill=tk.BOTH, expand=True)

        self.playlist_wrap = ttk.Frame(self.bottom_playlist_section.body); self._playlist_visible = True
        self.playlist_wrap.pack(fill=tk.BOTH, expand=True)
    ...
    def _populate_left_controls(self, parent):
        holder = ttk.Frame(parent)
        holder.pack(fill=tk.BOTH, expand=True, padx=4, pady=2)

        self.left_section_library = CollapsibleSection(holder, "Library", open_=True)
        self.left_section_library.pack(fill=tk.X)
        lib = self.left_section_library.body
        btn_open = tk.Button(lib, text="Open Folder [Ctrl+O]", command=self.open_folder)
        btn_open_json = tk.Button(lib, text="Open static.json [Ctrl+J]", command=self.open_static_file)
        btn_rescan = tk.Button(lib, text="Rescan [F5]", command=self.rescan_current_folder)
        for r, (b, role) in enumerate(((btn_open, "primary"), (btn_open_json, "secondary"), (btn_rescan, "accent"))):
            style_btn(b, role=role)
            b.grid(row=r, column=0, padx=2, pady=4, sticky="ew")
        lib.grid_columnconfigure(0, weight=1)

        self.left_section_view = CollapsibleSection(holder, "View", open_=True)
        self.left_section_view.pack(fill=tk.X, pady=(6, 0))
        view = self.left_section_view.body
        btn_img = tk.Button(view, text="Set Image [Ctrl+I]", command=self.choose_center_image)
        btn_img_rand = tk.Button(view, text="Random Image", command=self.shuffle_center_image)
        btn_playlist = tk.Button(view, text="Toggle Bottom [Ctrl+3]", command=self._toggle_playlist)
        btn_reset = tk.Button(view, text="Reset Layout", command=self.reset_panel_layout)
        for r, (b, role) in enumerate(((btn_img, "primary"), (btn_img_rand, "accent"), (btn_playlist, "secondary"), (btn_reset, "system"))):
            style_btn(b, role=role)
            b.grid(row=r, column=0, padx=2, pady=4, sticky="ew")
    ...
    def _populate_right_controls(self, parent):
        ...
        self.right_section_transport = CollapsibleSection(holder, "Playback", open_=True)
        self.right_section_transport.pack(fill=tk.X)
        play = self.right_section_transport.body
        btn_prev = tk.Button(play, text="Prev [Ctrl+←]", command=self.prev)
        btn_play = tk.Button(play, text="Play/Pause [Space]", command=self.toggle_play)
        btn_next = tk.Button(play, text="Next [Ctrl+→]", command=self.next)
        btn_stop = tk.Button(play, text="Stop [S]", command=self.stop)
        self._btn_repeat = tk.Button(play, text="Repeat: Off [R]", command=self.toggle_repeat)
        self._btn_shuffle = tk.Button(play, text="Shuffle: Off [H]", command=self.toggle_shuffle)
        for b, role in (
            (btn_prev, "secondary"),
            (btn_play, "primary"),
            (btn_next, "secondary"),
            (btn_stop, "system"),
            (self._btn_repeat, "secondary"),
            (self._btn_shuffle, "secondary"),
        ):
            style_btn(b, role=role)
        ...
        for c in range(3):
            play.grid_columnconfigure(c, weight=1)

        self.right_section_speed = CollapsibleSection(holder, "Speed", open_=True)
        self.right_section_speed.pack(fill=tk.X, pady=(6, 0))
        speed = self.right_section_speed.body
        self._btn_speed_dn = tk.Button(speed, text="0.1x −", command=lambda: self._bump_playback_rate(-0.1))
        self._btn_speed_1x = tk.Button(speed, text="1.0x", command=lambda: self.set_playback_rate(1.0))
        self._btn_speed_up = tk.Button(speed, text="0.1x +", command=lambda: self._bump_playback_rate(+0.1))
        self._btn_speed_fast = tk.Button(speed, text="2.0x", command=lambda: self.set_playback_rate(2.0))
        for b, role in (
            (self._btn_speed_dn, "accent"),
            (self._btn_speed_1x, "secondary"),
            (self._btn_speed_up, "accent"),
            (self._btn_speed_fast, "secondary"),
        ):
            style_btn(b, role=role)
        self._btn_speed_dn.grid(row=0, column=0, padx=2, pady=4, sticky="ew")
        self._btn_speed_1x.grid(row=0, column=1, padx=2, pady=4, sticky="ew")
        self._btn_speed_up.grid(row=0, column=2, padx=2, pady=4, sticky="ew")
        self._btn_speed_fast.grid(row=1, column=0, padx=2, pady=(4, 2), sticky="ew")
        self.speed_status_lbl = ttk.Label(speed, text="Speed: 1.0x")
        self.speed_status_lbl.grid(row=1, column=1, columnspan=2, padx=2, pady=(4, 2), sticky="w")
        self._refresh_transport_toggle_visuals()
        self._refresh_speed_label()
        for c in range(3):
            speed.grid_columnconfigure(c, weight=1)
    ...
    def _play_index(self, idx, resume_ms: int = 0):
        ...
        if vlc:
            ...
            self.root.after(250, self._apply_playback_rate)
            self.root.after(900, self._apply_playback_rate)
        self._refresh_speed_label()
    ...
    def toggle_sub_enabled(self):
        ...
        if not self.sub_enabled.get():
            ...
            self.subtitle_en_shadow_lbl.config(text="")
            self.subtitle_vi_shadow_lbl.config(text="")
        else:
            self._last_en_text = ""
            self._last_vi_text = ""
    ...
    def _update_subtitle_by_time(self, t_ms: int):
        ...
        if en_txt != self._last_en_text:
            txt = en_txt or ""
            self.subtitle_en_lbl.config(text=txt)
            self.subtitle_en_shadow_lbl.config(text=txt)
            self._last_en_text = txt
            self._relayout_sub_labels()

        vi_txt = self._lookup_cue_text(self.sub_vi_cues, t_ms_adj)
        if vi_txt != self._last_vi_text:
            txt = vi_txt or ""
            self.subtitle_vi_lbl.config(text=txt)
            self.subtitle_vi_shadow_lbl.config(text=txt)
            self._last_vi_text = txt
            self._relayout_sub_labels()

        if self.sub_en_cues:
            if not self.subtitle_en_box.winfo_ismapped():
                self.subtitle_en_box.pack(fill=tk.X, pady=(6, 0))
        else:
            if self.subtitle_en_box.winfo_ismapped():
                self.subtitle_en_box.pack_forget()

        if self.sub_vi_cues:
            if not self.subtitle_vi_box.winfo_ismapped():
                self.subtitle_vi_box.pack(fill=tk.X, pady=(2, 0))
        else:
            if self.subtitle_vi_box.winfo_ismapped():
                self.subtitle_vi_box.pack_forget()
    ...
    def _on_root_configure(self, event):
        try:
            ...
            self.subtitle_en_shadow_lbl.configure(wraplength=w)
            self.subtitle_vi_shadow_lbl.configure(wraplength=w)
            self._relayout_sub_labels()
        except Exception:
            pass
