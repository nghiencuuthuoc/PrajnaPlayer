# PrajnaPlayer using python-vlc (v8) + PharmApp Theme
# - v8 core:
#   * static.json ONLY inside the opened music folder (no script-side copy).
#   * Safe write for static.json using folder-local lock + atomic write.
#   * Per-folder state files next to script: state_<hash>.json . Global pointer "state_recent.json".
#   * CLI flag --read-only-static: never write static.json (Open/Rescan operate in-memory only).
#
# - UI Theme (PharmApp):
#   * Global bg #fdf5e6, text #2a2a2a; ttk 'default' theme.
#   * TFrame/TLabel bg #fdf5e6 (Label font Arial 11).
#   * Buttons: bg #f4a261, fg black, Arial 10 bold, no border; active bg #e76f51.
#   * Treeview: heading bg #b5838d (white), font Arial 10 bold;
#               body font Arial 10, rowheight 25, selection bg #e9c46a.
#   * TEntry/TCombobox padding=5.
#   * provide apply_pharmapp_theme(root) and call right after creating root.
...
def apply_pharmapp_theme(root: tk.Tk):
    """Apply PharmApp theme to Tk/ttk widgets."""
    # Global colors
    BG = "#fdf5e6"
    FG = "#2a2a2a"
    BTN_BG = "#f4a261"
    ...
    # TButton (for any ttk.Button you might add later)
    style.configure("TButton", font=("Arial", 10, "bold"), foreground="black", padding=5)
    style.map("TButton", background=[("active", BTN_ACTIVE), ("!active", BTN_BG)])

    # TEntry / TCombobox padding
    style.configure("TEntry", padding=5)
    style.configure("TCombobox", padding=5)

    # Treeview
    style.configure("Treeview", font=("Arial", 10), rowheight=25)
    style.configure("Treeview.Heading", background="#b5838d", foreground="white",
                    font=("Arial", 10, "bold"))
    style.map("Treeview",
              background=[("selected", "#e9c46a")],
...
class PrajnaPlayerUI:
    def __init__(self, sort_mode_arg=None, read_only_static=False):
        self.root = tk.Tk()
        apply_pharmapp_theme(self.root)  # <<< Apply theme immediately

        self.read_only_static = read_only_static  # v8
        ...
        # Optional logo from ./ (same folder as this script) — keep optional
        try:
            # logo_path = os.path.join(script_dir(), "phat_duoc_su_2.jpg")
            logo_path = os.path.join(script_dir(), "Shakaymuni_Buddha+17_Nalanda_Pandits.png")
        ...
        ttk.Label(col0, text="Search").pack()
        search_row = tk.Frame(col0)
        search_row.pack()
        # use ttk.Entry to get TEntry padding
        ent = ttk.Entry(search_row, textvariable=self.search_var, width=28)
        ...
        ttk.Label(col2, text="Sort by").pack()
    ...
    def open_folder(self):
        ...
        if os.path.exists(sp):
            if self.read_only_static:
                use_static = True  # read-only: always use existing if available
            else:
        ...
        if use_static:
            ...
            if not data:
                if self.read_only_static:
                    messagebox.showwarning("static.json", "static.json invalid and read-only mode is on.\nWill scan to memory (no write).")
                    self._scan_no_write(folder)
                else:
                    messagebox.showwarning("static.json", "static.json invalid. Will scan and write static.json.")
