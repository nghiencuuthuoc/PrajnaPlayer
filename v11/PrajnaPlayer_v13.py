...
# -*- coding: utf-8 -*-
"""
PrajnaPlayer v12.3 – Subtitles (.vtt/.srt) + Compact UI
- Hiển phụ đề đồng bộ thời gian (VTT/SRT) ngay dưới thanh progress
- Tự tìm file phụ đề khớp nhất trong folder (ưu tiên cùng tên; chứa cụm từ giống; chứa YouTube ID)
- Nút **Load VTT [Ctrl+L]** để chọn thủ công; **Sub On/Off [Ctrl+K]**
- Ảnh trung tâm như v12.2 (Set Image [Ctrl+I], nhớ trong prajna_config.json)
- Nút nhỏ + phím tắt thể hiện ngay trên nút
"""
...
def _norm(s: str) -> str:
    s = s.lower()
    s = re.sub(r"[^\w\s]", " ", s, flags=re.UNICODE)   # bỏ dấu câu
...
def parse_vtt_or_srt(path: Path) -> List[Tuple[int, int, str]]:
    ...
    # detect SRT by commas in time
    is_srt = any("-->" in ln and "," in ln for ln in lines[:20])
    cues: List[Tuple[int,int,str]] = []

    def to_ms_hhmmss(x: str) -> int:
        # supports 1:23:45.678 or 01:23:45.678 or 23:45.678
        x = x.strip()
        x = x.replace(",", ".")
        parts = x.split(":")
        if len(parts) == 2:
    ...
    for i, ln in enumerate(lines):
        if "-->" in ln:
            ...
            try:
                ...
                # remove after end time trailing meta
                right = right.strip().split(" ", 1)[0]
                t_end = to_ms_hhmmss(right.strip())
            except Exception:
                t_start = t_end = None
        else:
            # accumulate text lines (skip cue numbers in SRT)
    ...
    # sort & coalesce overlapping
    cues.sort(key=lambda x: x[0])
    return cues

def _cleanup_sub_text(txt: str) -> str:
    # remove simple tags (vtt <c> <b> etc.) and cues like <00:00:00>
    txt = re.sub(r"<[^>]+>", "", txt)
    # collapse spaces
    txt = re.sub(r"[ \t]+", " ", txt)
    # keep line breaks
...
class PrajnaPlayerApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("PrajnaPlayer v12.3 · PharmApp")
        ...
        self._build_subtitle_row()          # NEW: subtitle area
    ...
    def _build_subtitle_row(self):
        ...
        btn_load = tk.Button(top, text="Load VTT [Ctrl+L]", command=self.load_subtitle_manual)
        style_btn(btn_load)
        btn_load.pack(side=tk.LEFT, padx=(12, 6))

        self.btn_sub_toggle = tk.Button(top, text="Sub: On [Ctrl+K]", command=self.toggle_sub_enabled)
        style_btn(self.btn_sub_toggle)
        self.btn_sub_toggle.pack(side=tk.LEFT)

        # bottom: actual subtitle text
        self.subtitle_lbl = tk.Label(
            outer, text="", bg=BG, fg="#000000",
            font=("Arial", 13, "bold"), justify="center", wraplength=1200
        )
    ...
    def _auto_load_subtitle_for(self, audio_path: Path):
        ...
        for p in sorted(folder.glob("*")):
            ...
            if stem == mp3stem:
                score = 100.0
            elif norm.startswith(mp3norm) or norm.endswith(mp3norm):
                score = 90.0
            elif mp3norm and mp3norm in norm:
                score = 80.0
            else:
                tokens = _token_set(stem)
                inter = tokens & mp3tokens
                if mp3tokens:
                    score = 60.0 * (len(inter) / max(1, len(mp3tokens)))
            if yt_id and yt_id in p.name:
                score += 10.0
    ...
    def _update_subtitle_by_time(self, t_ms: int):
        if not self.sub_enabled.get() or not self.sub_cues or t_ms < 0:
            if self.sub_enabled.get():
                # when not playing, keep last line
                pass
            return
        ...
        if txt is None:
            # if between cues, clear slowly
            if self._last_sub_text and t_ms > 0:
                # keep last for a short while; do nothing
                return
            else:
                self.subtitle_lbl.config(text="")
                self._last_sub_text = ""
