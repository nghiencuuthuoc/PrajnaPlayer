...
# -*- coding: utf-8 -*-
"""
PrajnaPlayer v15.3 – Dual Subtitles (EN on top, VI below) + Pairing & Alignment Check
- Strict pair: BASE.en.vtt + BASE.vi.vtt (hoặc .srt). Các phần tên khác phải giống nhau.
- Nếu không có cặp strict → fallback ghép gần đúng.
- Kiểm tra khớp EN–VI: overlap (%) và median offset (Δ giây). Hiện "✓ aligned" hoặc "⚠ mismatch".
- Auto-next bền: bắt VLC EndReached + fallback near-end.
- Giữ tính năng từ v14: 2 phụ đề, A+/A−, Delay, nhớ config, playlist, hình trung tâm, phím tắt...
"""
...
class PrajnaPlayerApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("PrajnaPlayer v15.3 · PharmApp")
