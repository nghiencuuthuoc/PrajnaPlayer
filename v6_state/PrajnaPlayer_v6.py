# PrajnaPlayer using python-vlc (v7)
# - Changes in v7:
#   * REMOVE script-side copy ("<FolderName>_static.json") to avoid conflicts across multiple players.
#   * ONLY write static.json inside the opened music folder.
#   * Keep "Open static.json" button to load from any folder's static.json.
#
# - Kept from v6/v5:
#   * Optional logo loaded from ./ (same dir as script) if present.
#   * On "Open Folder": if static.json exists, ask to reuse or rescan/overwrite.
#   * Title uses static["title"] (default: folder name) => "<Title> || PrajnaPlayer VLC".
#   * "Rescan" button + F5 hotkey; overwrite static.json and try to restore current track/position.
#   * Columns: No., Title, Folder, Duration, Size, Modified; search, volume, sort; recurse subfolders.
#   * State persistence: state.json keeps last folder, volume, index, position.
...
def read_static(folder: str):
    """Read per-folder static.json."""
    sp = static_path_for(folder)
    return read_static_from_file(sp)

def read_static_from_file(path: str):
    """Read any static json by exact file path. Return dict or None."""
...
def write_static(folder: str, tracks: list, title: str):
    """
    Write ONLY the per-folder static.json (no script-side copy).
    Include base_folder for portability.
    """
    payload = {
        "title": title,
    ...
    try:
        with open(static_path_for(folder), "w", encoding="utf-8") as f:
            json.dump(payload, f, ensure_ascii=False, indent=2)
    except Exception as e:
        messagebox.showwarning("static.json", f"Cannot write static.json in folder:\n{e}")
...
def derive_base_folder_from_static(data: dict) -> str:
    """Prefer 'base_folder' if valid; else derive from the first track path."""
    base = data.get("base_folder", "") if isinstance(data, dict) else ""
    if base and os.path.isdir(base):
        return base
    # fallback: derive from first valid track path
    for item in data.get("tracks", []):
        p = item.get("path", "")
...
class PrajnaPlayerUI:
    def __init__(self, sort_mode_arg=None):
        ...
        tk.Button(fbtn, text="Open static.json", command=self.open_static_file).pack(side=tk.LEFT, padx=2)  # load from any folder's static.json
        tk.Button(fbtn, text="Rescan", command=self.rescan_current_folder).pack(side=tk.LEFT, padx=2)
    ...
    def open_static_file(self):
        """Pick a 'static.json' and load it."""
    ...
    def _scan_and_write_static(self, folder):
        ...
        write_static(folder, self.tracks, title)  # v7: ONLY write inside the folder
    ...
    def resort(self):
        ...
        if self.music_folder and os.path.isdir(self.music_folder):
            ...
            # Do not auto-write static here; use Rescan to persist.
    ...
    def load_previous_state(self):
        ...
        if folder and os.path.isdir(folder):
            ...
            print("Previous folder not found. Please open a new folder or static.json.")
    ...
    def rescan_current_folder(self):
        """Rescan current music folder, overwrite static.json, refresh list, and try to restore playback."""
        ...
        # Rescan and re-write static.json (folder-only)
