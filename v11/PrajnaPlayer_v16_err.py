...
# -*- coding: utf-8 -*-
"""
PrajnaPlayer v16.0 – Default Window Size + Dual Subtitles & Auto-next
- Mặc định cửa sổ: 1108x768 (như ảnh bạn gửi). Ghi nhớ geometry về sau.
- Giữ toàn bộ nâng cấp v15.3:
  • Ghép cặp phụ đề strict: BASE.en.vtt + BASE.vi.vtt (hoặc .srt); fallback fuzzy nếu không có strict.
  • 2 dòng phụ đề: EN (trên), VI (dưới); Delay toàn cục, thay đổi cỡ chữ.
  • Kiểm tra khớp EN–VI: overlap %, median offset Δs → "✓ aligned" / "⚠ mismatch".
  • Auto-next bền: VLC EndReached + near-end fallback.
  • Playlist, phím tắt, nhớ config, ảnh trung tâm, v.v.
"""
...
DEFAULT_GEOMETRY = "1108x768"   # <<< kích thước mặc định theo ảnh bạn gửi
...
class PrajnaPlayerApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("PrajnaPlayer v16.0 · PharmApp")
        ...
        # Apply geometry: nếu đã có geometry trong config thì dùng; nếu không, giữ giá trị mặc định đã set ở main()
    ...
    def _build_subtitle_row_dual(self):
        ...
        # Hai label phụ đề: EN (trên), VI (dưới)
    ...
    def load_subtitle_manual(self):
        ...
        if base_str:
            ...
            # nếu user mở 1 file, cố gắng ghép cặp đúng base
            ...
            # fallback: đoán ngôn ngữ và ghép gần đúng
    ...
    def _auto_load_subtitle_for(self, audio_path: Path):
        """Ưu tiên strict pair (BASE.en + BASE.vi). Nếu không có → fuzzy."""
        folder = audio_path.parent
        pairs = _pair_subtitles_in_folder(folder)
        en_path = vi_path = None
        if pairs:
            base = _closest_base_to_audio(list(pairs.keys()), audio_path.stem)
            if base:
                en_path = pairs[base].get("en")
                vi_path = pairs[base].get("vi")
        if not (en_path or vi_path):
            en_path, vi_path = self._fuzzy_pick_for_audio(audio_path)
    ...
    @staticmethod
    def _lookup_cue_text(cues: List[Tuple[int,int,str]], t_ms_adj: int) -> Optional[str]:
...
def main():
    root = tk.Tk()
    # Đặt kích thước mặc định NGAY TỪ ĐẦU (nếu không có geometry trong config, size này sẽ được giữ nguyên)
    try:
