...
# -*- coding: utf-8 -*-
"""
PrajnaPlayer v15.4 – Dual Subtitles + Exact Pairing by Track Name
- Ưu tiên cặp phụ đề đúng tên bài: <stem>.en.(vtt|srt) + <stem>.vi.(vtt|srt)
- Nếu thiếu 1 track → thử strict pair theo BASE.en/vi.(vtt|srt)
- Nếu không có strict → fuzzy gần đúng
- Kiểm tra khớp EN–VI: overlap (%) + median offset (Δ giây) → ✓ aligned / ⚠ mismatch
- Auto-next bền: VLC EndReached + fallback near-end
- Giữ tính năng v14/v15: 2 dòng phụ đề (EN trên, VI dưới), A+/A−, Delay, nhớ config, playlist, ảnh trung tâm, phím tắt...
"""
...
class PrajnaPlayerApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("PrajnaPlayer v15.4 · PharmApp")
    ...
    def _auto_load_subtitle_for(self, audio_path: Path):
        """Load phụ đề:
           1) exact <stem>.en/vi.(vtt|srt)
           2) strict pair BASE.en/vi.(vtt|srt) (base gần với tên audio)
           3) fuzzy gần đúng
        """
        folder = audio_path.parent

        # (1) Exact pair theo tên bài
        en_path, vi_path = self._exact_pair_for_audio(audio_path)
        if en_path or vi_path:
            # Nếu mới có 1 track, thử bổ sung track kia theo strict base cùng 'stem'
            pairs = _pair_subtitles_in_folder(folder)
            d = pairs.get(audio_path.stem, {})
            if d:
                en_path = en_path or d.get("en")
                vi_path = vi_path or d.get("vi")
    ...
    def _play_index(self, idx):
        ...
        # auto load dual subtitles for this track
