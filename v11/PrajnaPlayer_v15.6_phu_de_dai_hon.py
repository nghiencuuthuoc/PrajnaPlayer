...
# -*- coding: utf-8 -*-
"""
PrajnaPlayer v15.6 – Dual Subtitles + Global Sub-Index + Smart-Hold (MIN_HOLD + PER_CHAR + LINGER)
- Tự load phụ đề khi bấm bài mới / auto-next:
  1) <stem>.en/vi.(vtt|srt) cùng thư mục
  2) Strict pair BASE.en/vi.(vtt|srt) cùng thư mục
  3) Chỉ mục toàn thư mục đã mở (exact)
  4) Chỉ mục toàn thư mục đã mở (closest)
  5) Fallback fuzzy trong thư mục hiện tại
- Hai dòng phụ đề (EN trên, VI dưới)
- Kiểm tra khớp EN–VI (overlap %, median Δs)
- Smart-Hold: giữ phụ đề lâu hơn theo MIN_HOLD, PER_CHAR và LINGER (không tràn cue sau)
"""
...
class PrajnaPlayerApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("PrajnaPlayer v15.6 · PharmApp")
