...
# -*- coding: utf-8 -*-
"""
PrajnaPlayer v16 — Dual Subtitles + Global Sub-Index + Smart-Hold
+ Session resume (state_recent + state_<hash>.json)
+ Static cache (static.json in each music folder; atomic write with static.lock)
+ Option to disable writing static.json (Read‑Only Static)
Keeps all features of v15.6 (shortcuts, filters, pairing EN/VI, smart-hold…).

Author: PharmApp (www.nghiencuuthuoc.com / www.pharmapp.vn)
"""
...
class PrajnaPlayerApp:
        ...
        for pat in patterns:
            for p in old_dir.glob(pat):
                try:
                    ...
                    if not target.exists():  # tránh ghi đè nếu đã có
    ...
    def __init__(self, root: tk.Tk):
        ...
        # App data folder (chứa config + state)
        self.app_root = Path(__file__).parent
        self.app_dir  = self.app_root / "prajna_data"   # <--- thư mục riêng
        self.app_dir.mkdir(exist_ok=True)

        # Di chuyển (migrate) các file JSON cũ vào thư mục mới
        self._migrate_old_jsons(self.app_root, self.app_dir)

        # Config nằm trong thư mục data
        self.config_path = self.app_dir / "prajna_config.json"
        self._cfg = self._load_config()

        # Persistence flags/paths
        ...
        # SMART-HOLD params
        self.sub_linger_ms   = tk.IntVar(value=int(self._cfg.get("sub_linger_ms",   800)))   # thêm sau khi hết cue
        self.sub_min_hold_ms = tk.IntVar(value=int(self._cfg.get("sub_min_hold_ms", 1200)))  # tối thiểu mỗi cue
        self.sub_per_char_ms = tk.IntVar(value=int(self._cfg.get("sub_per_char_ms", 28)))    # ms/ký tự (không tính whitespace)
        ...
        # ---- Restore previous session (state + static) ----
    ...
    def _build_bottom_filters_bar(self):
        ...
        btn_img_rand = tk.Button(left, text="Random Img", command=self.shuffle_center_image)
        style_btn(btn_img_rand)
    ...
    def _find_default_image(self) -> Optional[Path]:
        ...
        candidates = [
            here / "phat_duoc_su_2.png", here / "Phat_Thich_Ca.jpg", here / "thien-thu-thien-nhan-bo-tat-7.jpg",
            here / "Quan_The_Am_Bo_Tat_TVVN_3.png",  here / "z_Dalai_Lama_+_Đức_Văn_Thù_Manjushri.jpg",
        ]
        assets = here / "assets"
        if assets.exists():
            for name in ["phat_duoc_su_2.png","Phat_Thich_Ca.jpg","thien-thu-thien-nhan-bo-tat-7.jpg","Quan_The_Am_Bo_Tat_TVVN_3.png","z_Dalai_Lama_+_Đức_Văn_Thù_Manjushri.jpg"]:
    ...
    def _load_center_image(self, parent):
        ...
        # Ưu tiên ảnh random trong assets; nếu không có thì fallback
    ...
    def _items_from_static(self, data: dict) -> List[Dict]:
        ...
        try:
            for t in data.get("tracks", []):
                p = Path(t.get("path",""))
                if not p.exists():  # skip missing files
                    continue
                items.append({
                    "path": str(p),
    ...
    def open_folder(self):
        ...
        # After first load, try to auto-play from state for this folder
    ...
    # ---- NEW: play sequence helpers & next/prev using view ----
    def _play_sequence(self) -> List[int]:
        """Return the sequence (indices in items_all) that playback should follow."""
        if self.items_view:
    ...
    def _next_index(self):
        ...
        # Shuffle: random over seq, avoid repeating current if possible
        if self.is_shuffle:
            if len(seq) == 1:
                return seq[0]
        ...
        if self.current_index in seq:
            ...
            # current not in seq (filter changed): start from first
            return seq[0]
    ...
    def _update_progress(self):
        ...
        if (state == vlc.State.Ended or near_end) and not getattr(self, "_end_fired", False):
            # decide by sequence (items_view aware)
            seq = self._play_sequence()
    ...
    def _restore_last_session(self):
        ...
        # volume first
        self.volume.set(int(st.get("volume", 70)))
        # find index: prefer exact song path, else index
        idx = None
        if st.get("song") and Path(st["song"]).exists():
            idx = self._index_of_path(st["song"])
