# PrajnaPlayer_VLC_PAT.py - PrajnaPlayer with python-vlc and PharmApp Theme
import os
import json
import random
import threading
...
# Optional: Apply PharmApp Theme
try:
    from theme_pharmapp_tk import apply_pharmapp_theme
except ImportError:
    apply_pharmapp_theme = None
...
class PrajnaPlayerUI:
    def __init__(self):
        ...
        # Apply PharmApp theme if available
        if apply_pharmapp_theme:
        ...
        self.sort_dropdown = ttk.Combobox(sort_frame, textvariable=self.sort_mode, values=[
            "Newest First", "Filename Z-A", "Filename A-Z", "Oldest First",
    ...
    def load_music(self, folder):
        ...
        elif sort_mode == "Size: Largest First":
            full_paths.sort(key=lambda x: os.path.getsize(x), reverse=True)
        elif sort_mode == "Size: Smallest First":
            full_paths.sort(key=lambda x: os.path.getsize(x))
        else:
            full_paths.sort(key=lambda x: int(''.join(filter(str.isdigit, os.path.basename(x))) or 0))

        self.media_list = full_paths
