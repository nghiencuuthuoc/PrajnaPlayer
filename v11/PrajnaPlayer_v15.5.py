...
# -*- coding: utf-8 -*-
"""
PrajnaPlayer v15.5 – Dual Subtitles + Global Sub-Index (auto-load on next/prev/auto-next)
- Ưu tiên cặp <stem>.en.(vtt|srt) + <stem>.vi.(vtt|srt) cùng thư mục
- Strict pair BASE.en/vi trong thư mục
- Tìm phụ đề theo chỉ mục toàn thư mục đã mở (exact rồi closest)
- Fallback fuzzy trong thư mục
- Kiểm tra khớp EN–VI (overlap %, median Δs), auto-next bền
"""
...
class PrajnaPlayerApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("PrajnaPlayer v15.5 · PharmApp")
    ...
    def open_static_file(self):
        ...
        # xây sub-index từ gốc thư mục của file đầu tiên
    ...
    def _play_index(self, idx):
        ...
        # auto load dual subtitles (now with global sub-index)
